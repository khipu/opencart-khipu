...
class ControllerExtensionPaymentKhipuWebpay extends Controller {
	...
	public function process() {
        ...
        $data['receiver_id'] = $this->config->get('payment_khipu_webpay_receiverid');
        ...
        $data['notify_url'] = $this->url->link('extension/payment/khipu_webpay/callback', '', 'SSL');
        ...
        try {
            $createPaymentResponse = khipu_create_payment($this->config->get('payment_khipu_webpay_receiverid')
                , $this->config->get('payment_khipu_webpay_secret')
                , $data);
        ...
        $this->response->redirect($createPaymentResponse->getWebpayUrl());

	}


	public function index() {
		$this->language->load('extension/payment/khipu_webpay');
		...
		if ($order_info) {
			...
			$data['action'] = $this->url->link('extension/payment/khipu_webpay/process', '', 'SSL');
            return $this->load->view('extension/payment/khipuwebpay', $data);
		}
	}

	public function callback() {
		$payment = khipu_get_payment($this->request->post['api_version'], $this->config->get('payment_khipu_webpay_receiverid'), $this->config->get('payment_khipu_webpay_secret'), $this->request->post);
        ...
        $total = $this->currency->format($order_info['total'], $order_info['currency_code'], false, false);
        if($payment->getReceiverId() == $this->config->get('payment_khipu_webpay_receiverid')
            && $payment->getStatus() == 'done'
            && $total == $payment->getAmount()
            && $order_info['currency_code'] == $payment->getCurrency()
        ) {
            $this->model_checkout_order->addOrderHistory($payment->getCustom(), $this->config->get('payment_khipu_webpay_completed_status_id'));
        } else {
            error_log("invalid response\n");
        }
	}
