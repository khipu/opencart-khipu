...
class ControllerPaymentKhipuManual extends Controller {
	...
	public function process() {
		$json_string = khipu_create_payment($this->config->get('khipu_manual_receiverid')
					...
					, $this->request->post
					, 'opencart-khipu-manual-1.5.4;;'.$this->config->get('config_url').';;'.$this->config->get('config_name'));
	...
	protected function index() {
		...
		if ($order_info) {
			...
			$banks = khipu_get_available_banks($this->data['receiver_id'], $this->config->get('khipu_manual_secret'), 'opencart-khipu-manual-1.5.4;;'.$this->config->get('config_url').';;'.$this->config->get('config_name'));
